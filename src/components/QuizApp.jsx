...
const QuizApp = ({ data }) => {
  ...
  const handelOptionClick = (answer, isCorrect) => {
    setSelectedOption(answer);

    if (!answer) return;
    setCorrectAnswers((prev) => {
      const cloneData = [...prev];
      cloneData.push(isCorrect);
      return cloneData;
    });
  };
      ...
      {!isFormCompleted ? (
        <>
          ...
          <div className="btns text-right">
            ...
            onClick={submitQuestions} className=" mx-6 disabled:bg-white/20 disabled:cursor-not-allowed">
              {data.length - 1 === currentQuestion ? (
                <p className={`px-10 bg-teal-700 py-2 ${!selectedOption && 'bg-white/20'}`}>Submit</p>
              ) : (
